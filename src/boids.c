...
void InitBoids()
{
    for (int i = 0; i < MAX_BOIDS; i++) {
        ...
        boids[i].velocity = Vector2Scale((Vector2){ cosf(angle), sinf(angle) }, 4.0f);
...
void DrawBoid(Boid boid) {
    ...
    // Draw main circle
    DrawCircleLinesV(boid.position, PROTECTED_RADIUS, boid.predated ? GREEN : RED);
...
void DrawBoids()
{
    ...
    for (int i = 0; i <= MAX_BOIDS; i++) {
        if (boids[i].isPredator) {
            DrawCircleV(boids[i].position, 10, YELLOW);
        } else {
